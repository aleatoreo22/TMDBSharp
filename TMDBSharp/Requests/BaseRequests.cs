...
{
    internal static async Task<R?> RequestAsync<R>(
        string endPoint,
        HttpMethod method,
        Dictionary<string, object?>? parameters = null
    ) => await RequestAsync<object, R>(null, endPoint, method, parameters);

    internal static async Task<R?> RequestAsync<T, R>(
        T? model,
        string endPoint,
        HttpMethod method,
        Dictionary<string, object?>? parameters = null
    )
    {
        ...
        if (parameters != null && parameters.Count > 0)
            endPoint = parameters.Aggregate(
                endPoint,
                (current, item) => current + "?" + item.Key + "=" + item.Value?.ToString()
            );
    ...
    internal static Dictionary<string, object?> FillBaseparameters(
        int? page = null,
        string? language = null,
        string? sort_by = null,
        bool? include_adult = null,
        bool? include_video = null,
    ...
    )
    {
        var parameters = new Dictionary<string, object?>
        {
            ...
            { nameof(append_to_response), append_to_response },
        };
        if (with_release_type != null && with_release_type.Count > 0)
            parameters.Add(
                ...
                string.Join(
                    "|",
                    with_release_type?.Where(x => x != null)?.Select(x => x.GetHashCode())
                )
            );
        return parameters;
    }

    internal static Dictionary<string, object?>? RemoveNullparameters(
